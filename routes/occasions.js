...
router.put('/:occasionSlug', ensure.ensureLoggedIn({redirectTo: '/users/sign-in'}), urlencodedParser, function (req, res) {
...
/* DELETE occasion.*/
router.delete('/:occasionSlug', ensure.ensureLoggedIn({redirectTo: '/users/sign-in'}), function (req, res) {
...
router.post('/:occasionSlug/share', urlencodedParser, ensure.ensureLoggedIn({redirectTo: '/users/sign-in'}), function (req, res) {
	...
	usersService.findById(req.user._id).then(function (user) {
		if (user) {
			sendService.sendEmail({
				...
				occasionSharesService.create(req.params.occasionSlug, req.body.emails).then(function () {
					res.status(200);
